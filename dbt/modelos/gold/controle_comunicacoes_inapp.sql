{{ standard_config(
	model_name='controle_comunicacoes_inapp',
	zone='gold',
	materialized='table'
) }}
...
with  nomes_disparos as (
    ...
    DAY(dt_envio) AS dia_disparo,
    MONTH(dt_envio) AS mes_disparo,
    YEAR(dt_envio) AS ano_disparo,
    regexp_extract(TRIM(messagename), '^(.*?)_', 1) AS centro_custo,
    regexp_extract(TRIM(messagename), '_([^_]+)_', 1) AS area_disparadora,
