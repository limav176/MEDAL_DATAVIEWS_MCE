{{ standard_config(
	model_name='comparativo_camadas_push', 
	zone='gold',
	materialized='table'
) }}


WITH push_silver_bronze AS (
    SELECT 
       dt_disparo, 
       total_silver,
       total_bronze,
       diff_silver_bronze
    FROM 
      {{ ref('comparativo_push') }} 
    --  comunicacoes_silver.comparativo_push
)
,push_gold AS (
    SELECT 
        date(dt_envio) AS dt_disparo, 
        count(DISTINCT id_push) AS total_gold
    FROM 
          {{ ref('retorno_push_detalhado') }} 
       -- comunicacoes_gold.retorno_push_detalhado
    WHERE 
...
FROM 
    push_silver_bronze s
FULL OUTER JOIN 
    push_gold g ON g.dt_disparo = s.dt_disparo
ORDER BY 
    s.dt_disparo DESC